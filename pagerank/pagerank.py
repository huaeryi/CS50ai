...
def transition_model(corpus, page, damping_factor):
    ...
    if len(corpus[page]) == 0:
        prob = dict.fromkeys(corpus.keys(), 1 / len(corpus))
    else:
        prob = dict.fromkeys(corpus.keys(), (1 - damping_factor) / len(corpus))
        for page in corpus[page]:
            prob[page] += damping_factor / len(corpus[page])
    return prob
...
def sample_pagerank(corpus, damping_factor, n):
    ...
    pagerank[nextpage] += 1
    for i in range(n - 1):
        prob = transition_model(corpus, nextpage, damping_factor)
        choose = random.uniform(0, 1)
        sum = 0
        for page, rank in prob.items():
            sum += rank
            if sum > choose:
                break
        nextpage = page
