...
class MinesweeperAI():
    ...
    def conclude(self):
        ...
        for sentence in self.knowledge:
            if len(sentence.known_mines()) != 0:
                newclue += 1
                minecells += list(sentence.known_mines())
            if len(sentence.known_safes()) != 0:
                newclue += 1
                safecells += list(sentence.known_safes())
        if len(minecells) > 0:
        ...
        for i in range(len(self.knowledge)):
            for j in range(i + 1, len(self.knowledge)):
                if self.knowledge[i].cells < self.knowledge[j].cells:
                    clue = Sentence(self.knowledge[j].cells - self.knowledge[i].cells,
                    ...
                    if not flag:
                        continue
                    newclue += 1
                    new_knowledge.append(clue)
                elif self.knowledge[i].cells > self.knowledge[j].cells:
                    clue = Sentence(self.knowledge[i].cells - self.knowledge[j].cells,
    ...
    def add_knowledge(self, cell, count):
        ...
        minecount = len(neighbors & self.mines)
        neighbors -= self.mines
        self.knowledge.append(Sentence(neighbors, count - minecount))
        while True:
            newconclude = self.conclude()
            newinfer = self.infer()
            if newconclude == newinfer == 0:
                break
